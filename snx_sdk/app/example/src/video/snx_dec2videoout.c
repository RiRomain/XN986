...
static const char short_options[] = "hW:H:c:s:o:B:x:y:l:g:";
...
static const struct option long_options[] = {
  ...
  {"out_x", required_argument, NULL, 'x'},
	{"out_y", required_argument, NULL, 'y'},
	{"out_w", required_argument, NULL, 'l'},
	{"out_h", required_argument, NULL, 'g'},
	{NULL, 0, NULL, 0}
};
...
static void usage(FILE *fp, int argc, char **argv)
{
	fprintf(fp, "Usage: %s [options] input\n"
        ...
        "-x, --output x value. (default=0)\n"
		    "-y, --output y value. (default=0)\n"
		    "-l, --output width\n"
		    "-g, --output height\n"
		    "", argv[0]);
}
...
int main(int argc, char **argv)
{
  ...
  int out_x=0,out_y=0,out_w=640,out_h=480;
	...
	while(1) {
		...
		switch(c)
		{
			...
			case 'x':
	      sscanf(optarg, "%d", &out_x);
				break;
			case 'y':
         
				 sscanf(optarg, "%d", &out_y);
         
				break; 
      case 'l':
				sscanf(optarg, "%d", &out_w);
				break;
			case 'g':        
				sscanf(optarg, "%d", &out_h);
	...
	ctx->output_mode = MODE_LCD; //MODE_PRINT_SCREEN; //MODE_DUMP_FILE; //MODE_LCD;
...
#define VO_DEV_NAME "/dev/video3"  
  ...
  m2m->in_w = w/scale_ratio;
  m2m->in_h = h/scale_ratio;
  m2m->out_x = out_x;
  m2m->out_y = out_y;
  m2m->out_w = out_w;
  m2m->out_h = out_h;
