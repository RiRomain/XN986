...
static const char short_options[] = "hW:H:c:s:o:B:x:y:n:v:r:";
...
static const struct option long_options[] = {
  ...
  {"_x", required_argument, NULL, 'x'},
	{"_y", required_argument, NULL, 'y'},
	{"_sw", required_argument, NULL, 'n'},
	{"_sh", required_argument, NULL, 'v'},
  {"_rate", required_argument, NULL, 'r'},
	{NULL, 0, NULL, 0}
};
...
static void usage(FILE *fp, int argc, char **argv)
{
	fprintf(fp, "Usage: %s [options] input\n"
		    ...
		    "-o, --output filename\n\tOutput file name. (default=test.yuv)\n\tlcd\n"
		    "-B, --buffer\t\tV4L2 buffer. (default=2)\n\n"
        "-x, --x. output x(default=0)\n\n"
        "-y, --y. output y(default=0)\n\n"
        "-n, --n. output width(default=320)\n\n"
        "-v, --v. output height(default=480)\n\n"
        "-r, --r. frame rate(default=30)\n\n"
		    "input:\n\tfile path.\n\tuvcvideo\n\n"
		    "", argv[0]);
}
#if USE_TERMIOS
...
void *mjpeg_process_thread(void *arg)
{
  ...
  struct timeval s1_tv, e1_tv, res1_tv;
	...
	for (;;) {
...
#if 0 // verify bit-stream reader in memory mode w/ h.264
		char *fname = NULL, *raw_buf = NULL;
		FILE *fp = NULL;

		fname = calloc(sizeof(char), 256);

		sprintf(fname, "Frame_%04u.h264", fileid++);
		if (fileid == 150)
			fileid = 0;
		...
		raw_buf = malloc(file_sz);
		ret = fread(raw_buf, 1, file_sz, fp);
		if (ret < file_sz) {
...
#if 1
#if _DEBUG_TIME
      gettimeofday(&s1_tv, NULL);
#endif      
			ret = snx_bs_reader_read_frame(m2m,
					ctx->uvc_cap_buffers[ctx->uvc_index].start,
					buf.bytesused,
					m2m->dec_out_buffers[m2m->dec_out_index].start,
					m2m->dec_out_buffers[m2m->dec_out_index].length);
#if _DEBUG_TIME          
       gettimeofday(&e1_tv, NULL);
      timersub(&e1_tv, &s1_tv, &res1_tv);
		  printf("parser takes %ld ms frame %d, size %u\n",
				((res1_tv.tv_sec*1000)+(res1_tv.tv_usec/1000)),frame_count, ret); 
#endif           
#else
			memcpy(m2m->dec_out_buffers[m2m->dec_out_index].start,
				ctx->uvc_cap_buffers[ctx->uvc_index].start,
				buf.bytesused);
			ret = buf.bytesused;
#endif
		...
		if (ret == -1) { // EOF
			...
			// bit-stream parser in memory mode but frame parsing failed
			printf("### drop frame#%d ###\n", frame_count);
			frame_count++;
...
//		printf("bit stream size %d\n", m2m->dec_out_bytesused);
//		hexdump(m2m->dec_out_buffers[m2m->dec_out_index].start, 64);
//		hexdump(m2m->dec_out_buffers[m2m->dec_out_index].start +
//				m2m->dec_out_bytesused - 64, 64);
    ...
    gettimeofday(&e1_tv, NULL);
      timersub(&e1_tv, &s1_tv, &res1_tv);
		  printf("decode takes %ld ms \n",
				((res1_tv.tv_sec*1000)+(res1_tv.tv_usec/1000)));
#endif        
		if (ret < 0) {
		...
		if (ctx->step) {
			...
			printf("frame%d press 'space' for next frame, or 'q' to quit\n",
					frame_count);
...
void *overlay_thread(void *arg)
{
	...
	printf("============snx_m2m_thread============\n");
	if (m2m->m2m) {
		m2m->isp_fd = snx_open_device(m2m->isp_dev);
		ret = snx_isp_init(m2m);
		if (ret < 0) {
			perror("isp init failed");
			goto failed;
		}
		snx_isp_start(m2m);
		printf("============snx_m2m_thread============  isp\n");
	}
	...
	while(1) {
		...
		CLEAR(buf);
		buf.index = m2m->isp_index;
		buf.type = V4L2_BUF_TYPE_VIDEO_OUTPUT;
		buf.memory = V4L2_MEMORY_USERPTR;
		buf.m.userptr = m2m->isp_buffers[buf.index].start;
		buf.length = m2m->isp_buffers[buf.index].length;
		...
		if (ret < 0) {
			perror("dsp dqbuf failed");
			goto failed;
		}

		ret = snx_isp_reset(m2m);
		if (ret < 0) {
			perror("isp reset failed");
			goto failed;
		}
	}
...
static int uvc_init(snx_context *ctx)
{
	...
	parm.parm.capture.timeperframe.denominator = ctx->_rate;
...
int main(int argc, char **argv)
{
  ...
  g_ctx = ctx;
	...
	// function test, HFLIP and VFLIP
	if (0) {
		struct v4l2_control ctrl;
		CLEAR(ctrl);
		ctrl.id = V4L2_CID_VFLIP;
		ctrl.value = 1;
		ret = ioctl(m2m->dec_fd, VIDIOC_S_CTRL, &ctrl);
		if (ret < 0)
			perror("horizontal flip");
	}
