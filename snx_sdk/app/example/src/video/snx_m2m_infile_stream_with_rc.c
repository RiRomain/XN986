...
#define WIDTH			320				// Video Encoded Frame Width
#define HEIGHT			240					// Video Encoded Frame Height
...
#define FRAME_NUM		10  				// Video Encoded Frame Number
#define SCALE			1					// 1: 1, 2: 1/2, 4: 1/4 scaling down
#define INPUT_FILE		"/tmp/320x240_10fps.yuv"	// Video Encoded File Path
#define FILENAME		"/mnt/test/video1.h264"	// Video Encoded File Path
#define QP				29					// Set QP 10 - 50 (10: high quality/bitrate , 50 : low quality)
#define BIT_RATE		512000				// Bitrate control for H264 ONLY
#define DS_DEV_NAME		"1_h"				// Data stamp Device name for MROI setting
...
//	strcpy(m2m->isp_dev,IMG_DEV_NAME);
	...
	if(strlen(INPUT_FILE))
		stream1->in_fd = open(INPUT_FILE, O_RDONLY | O_NONBLOCK | O_CREAT);
	...
	/*--------------------------------------------------------
		Start 
	---------------------------------------------------------*/
	ret = pthread_create(&stream1->stream_thread, NULL, (void *)snx_m2m_infile_rc_flow, stream1);
	if(ret != 0) {
