...
#define FRAME_NUM		300000					// Video Encoded Frame Number
#define SCALE			1					// 1: 1, 2: 1/2, 4: 1/4 scaling down
//#define FILENAME		"/tmp/video1.h264"	// Video Encoded File Path
#define FILENAME		"/media/mmcblk0p1/video1.h264"
#define BIT_RATE		4000000				// Bitrate control for H264 ONLY
...
int main(int argc, char **argv)
{
	...
	strcpy(m2m->ds_dev_name, DS_DEV_NAME);

	strcpy(m2m->isp_dev,IMG_DEV_NAME);
	strcpy(m2m->codec_dev,CAP_DEV_NAME);
	
	if(strlen(file))
		stream1->fd = open(file, O_RDWR | O_NONBLOCK | O_CREAT | O_TRUNC );
