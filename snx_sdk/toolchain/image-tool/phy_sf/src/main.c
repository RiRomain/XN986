  ...
  //read flash layout file
	...
	while(fgets(line,READ_LENGTH,con_fp)) {
		...
		while(t != NULL){
			...
			if(section_num == 3){
				sprintf(file,"%s",t);
				if(file[strlen(file) - 1] == 0xd) file[strlen(file) - 1] = '\0';
				if(strcmp(type,rootfs_flag) == 0) {  
					if(!(find_size=fopen(file,"rb"))) {
	    					printf("Can not open '%s'!\n",file);
	    					return -1;
					} 
					fseek(find_size, 0, SEEK_END);
					if((rootfs_r_size=ftell(find_size))==-1) {
      						printf("Sorry! Can not calculate '%s' which larger than 2 GB!\n",file);    
      						fclose(find_size);
     						return -1;
					} 
					fclose(find_size);
...
#if defined(CONFIG_SYSTEM_PLATFORM_ST58660FPGA) || defined(CONFIG_SYSTEM_PLATFORM_SN98660)
#else
					if(address_end[strlen(address_end) - 1] == '\n') address_end[strlen(address_end) - 1] = '\0';
					rootfs_info_addr = (unsigned int)strtoll(address_end, NULL, 0);
					rootfs_info_addr -= 3;
#endif
				}
//#ifdef CONFIG_SYSTEM_PLATFORM_ST58660FPGA
#if defined(CONFIG_SYSTEM_PLATFORM_ST58660FPGA) || defined(CONFIG_SYSTEM_PLATFORM_SN98660)
				else if ((strcmp(type,jffs2_flag) == 0) && (strcmp(address_start,address_end) != 0)) { 
					if(!(find_size=fopen(file,"rb"))) {
	    					printf("Can not open '%s'!\n",file);
	    					return -1;
					} 
					fseek(find_size, 0, SEEK_END);
					if((jffs2_size=ftell(find_size))==-1){
      						printf("Sorry! Can not calculate '%s' which larger than 2 GB!\n",file);    
      						fclose(find_size);
     						return -1;
					} 
	...
	/*
	change phy_user.conf filling size for rootfs-r/rootfs-r-2
	this changes has been processed in the step 'deal_with_insertdata'
	*/
	//printf("%s\n",rootfs_end_address_1);
	//printf("%s\n",rootfs_end_address_2);
	
	
	/**
   	 * read user conf file and do work
   	 */
	#if 0
		if(deal_with_insertdata(user_conf,out_file,rootfs_r_size, jffs2_size)){
			fprintf(stderr, "process INSERT_DATA error\n");
			return -1;			
		}
	#endif
//#ifdef CONFIG_SYSTEM_PLATFORM_ST58660FPGA
#if defined(CONFIG_SYSTEM_PLATFORM_ST58660FPGA) || defined(CONFIG_SYSTEM_PLATFORM_SN98660)
		printf ("insert data addr=0x%x, size=0x%x\n", rootfs_info_addr, rootfs_r_size);
		if(insert_data(out_file,rootfs_info_addr, 4, rootfs_r_size)){
			fprintf(stderr, "process INSERT_DATA error\n");
			return -1;			
		}
		if (jffs2_size > 0) {
			if(insert_data(out_file,jffs2_info_addr, 4, jffs2_size)){
				fprintf(stderr, "process INSERT_DATA error\n");
				return -1;			
			}
		}
